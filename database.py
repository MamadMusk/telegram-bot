...
def get_users_paginated(page: int = 1, size: int = 50, search: str = "") -> Tuple[List[Dict[str, Any]], int]:
...
def get_downloads_paginated(page: int = 1, size: int = 50) -> Tuple[List[Dict[str, Any]], int]:
    offset = max(0, (page - 1) * size)
    with get_conn() as conn:
