...
Includes: users, downloads, settings, daily_quota (rate limiting).
...
def init_db() -> None:
    ...
    with get_conn() as conn:
        ...
        defaults: List[Tuple[str, str]] = [
            ...
            ("is_active", "True"),
            ("max_file_size", str(settings.MAX_FILE_SIZE_MB)),
            ("daily_quota", str(settings.DAILY_QUOTA)),
...
def check_quota(user_id: int) -> Tuple[bool, int, int]:
    ...
    limit_str = get_setting("daily_quota", str(settings.DAILY_QUOTA))
    limit = int(limit_str) if limit_str else settings.DAILY_QUOTA
